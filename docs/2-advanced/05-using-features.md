...
| `main`         | `ui5-li-group-header`                             | dynamically loaded if `wrappingType="Normal"` is set (to pre-load: `import "@ui5/webcomponents/dist/features/ListItemGroupHeaderExpandableText.js"` )  | Adds support for expandable text in list item group header when wrapping type is set to "Normal" mode.              |
