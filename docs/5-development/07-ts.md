...
- Note the difference between Typescript types (`string`, `boolean`) and Javascript primitives (`String`, `Boolean`).
...
### Defining events
 ...
 - The `@event` decorator must be used outside the class (contrary to `@property` and `@event`).
