...
@customElement({
	...
	templates: MyElementTemplate,
})
class MyElement extends UI5Element {
	...
}
```
And the pattern goes on for all the other `static get` methods as well, such as :
 - `static get render()`;
   - **Note!**: We now use it as a **`renderer`** instead of `render` in the `@customElement` decorator;
