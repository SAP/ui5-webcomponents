...
In the module, where you are using the components, you should add  ```APP_INITIALIZER```. In order to do so, import ```APP_INITIALIZER``` and add it to the providers array like this. In this example we will add it in the ```app.module.ts```:
```js
