 ...
 */
const modifyDateBy = (date, amount, unit, primaryCalendarType = "Gregorian", minDate = null, maxDate = null) => {
	...
	if (minDate && newDate.valueOf() < minDate.valueOf()) {
		return new CalendarDate(minDate, primaryCalendarType);
	}

	if (maxDate && newDate.valueOf() > maxDate.valueOf()) {
		return new CalendarDate(maxDate, primaryCalendarType);
	}
