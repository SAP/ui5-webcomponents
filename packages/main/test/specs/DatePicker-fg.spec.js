...
describe('Date Picker Field Glass modifications', () => {
	it('direct usage for comparison', () => {
		...
		const picker = browser.$(browser.getStaticAreaRespPopover(`#ui5-datepicker--startDate`));
		assert.ok(!picker.getProperty("opened"), "popover is initially hidden");

		const innerInput = browser.$("#ui5-datepicker--startDate").shadow$("ui5-input").shadow$("input");
		innerInput.click();
		assert.ok(picker.getProperty("opened"), "popover is visible");

		innerInput.keys("Tab");
		const dpEnd = browser.$("#ui5-datepicker--endDate");
		assert.ok(!picker.getProperty("opened"), "popover is hidden");
