...
describe("Clock API", () => {
	...
	it("'displayStep' and 'valueStep' properties", async () => {
		...
		const numbersInHours12 = await hours12.shadow$$(".ui5-tp-clock-items .ui5-tp-clock-item:not(.ui5-tp-clock-item-with-marker) .ui5-tp-clock-number");
		const numbersInMinutes = await minutes.shadow$$(".ui5-tp-clock-items .ui5-tp-clock-item:not(.ui5-tp-clock-item-with-marker) .ui5-tp-clock-number");
		const numbersInMinutes10 = await minutes10.shadow$$(".ui5-tp-clock-items .ui5-tp-clock-item:not(.ui5-tp-clock-item-with-marker) .ui5-tp-clock-number");
	...
	it("'lastItemReplacement' and 'prependZero' properties", async () => {
		...
		const minutes = await browser.$("#myMinutes");
		const numbersInHours24 = await hours24.shadow$$(".ui5-tp-clock-items .ui5-tp-clock-item:not(.ui5-tp-clock-item-with-marker) .ui5-tp-clock-number");
		const numbersInMinutes = await minutes.shadow$$(".ui5-tp-clock-items .ui5-tp-clock-item:not(.ui5-tp-clock-item-with-marker) .ui5-tp-clock-number");

		assert.strictEqual(await numbersInHours24[numbersInHours24.length-1].getText(), "0", "The last number element in clock with prependZero=false and no lastItemReplacement property set is '24'");
...
describe("Clock item selection", () => {
	it("select clock item and 'change' event", async () => {
		...
		await browser.pause(500);
		assert.strictEqual(await hours24.getProperty("selectedValue"), 22, "The selected value is proper");
		assert.strictEqual(Number(parseInt(await valueInput.getValue())), 22, "The event returned proper value");
