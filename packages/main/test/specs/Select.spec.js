...
describe("Select general interaction", () => {
	...
	it("remains closed and unchanged when read-only", async () => {
		...
		// assert
		assert.notOk(await popovergetProperty("open"), "Select remains closed.");
	...
	it("opens upon space", async () => {
		...
		assert.ok(await popovergetProperty("open"), "Select is opened.");
	});
	...
	it("toggles upon F4", async () => {
		...
		await browser.keys("F4");
		assert.notOk(await popovergetProperty("open"), "Select is closed.");
	});
