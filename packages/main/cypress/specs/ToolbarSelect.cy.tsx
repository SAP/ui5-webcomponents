...
describe("Toolbar general interaction", () => {
	...
	it("Should fire change event on selection change", () => {
		...
		cy.mount(
			<>
				<Toolbar>
					<ToolbarSelect onChange={handleChange}>
						<ToolbarSelectOption>1</ToolbarSelectOption>
						<ToolbarSelectOption selected>2</ToolbarSelectOption>
						<ToolbarSelectOption>3</ToolbarSelectOption>
					</ToolbarSelect>
				</Toolbar>
				<input placeholder="Changed" id="selectResult" />
			</>
		);

		cy.get("ui5-toolbar-select").then(($select) => {
			$select[0].addEventListener("ui5-change", (e) => {
				const input = document.getElementById("selectResult") as HTMLInputElement;
				input.value = "1";
			});
		});
		...
		cy.get("#selectResult").should("have.prop", "value", "1");
	});
});