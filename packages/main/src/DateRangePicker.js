...
class DateRangePicker extends DatePicker {
	...
	async _modifyDateValue(amount, unit) {
		...
		if (caretPos <= this.value.indexOf(this._effectiveDelimiter)) { // The user is focusing the first date -> change it and keep the seoond date
			const firstDateModified = modifyDateBy(new Date(this._firstDateTimestamp * 1000), amount, unit, this.getFormat().parse(this._minDate), this.getFormat().parse(this._maxDate));
			...
			const lastDateModified = modifyDateBy(new Date(this._lastDateTimestamp * 1000), amount, unit, this.getFormat().parse(this._minDate), this.getFormat().parse(this._maxDate));
