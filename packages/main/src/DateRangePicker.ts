...
class DateRangePicker extends DatePicker {
	...
	get _tempTimestamp() {
		return this._tempValue && (this.getFormat().parse(this._tempValue, true, undefined) as Date).getTime() / 1000;
	}
	...
	_extractFirstTimestamp(value: string) {
		...
		const parsedDate = this.getFormat().parse(dateStrings[0], true, undefined) as Date;
	...
	_extractLastTimestamp(value: string) {
		...
		if (dateStrings[1]) {
			const parsedDate = this.getFormat().parse(dateStrings[1], true, undefined) as Date;
			return parsedDate.getTime() / 1000;
		}
