...
class Suggestions {
	...
	async open() {
		...
		this.responsivePopover?.showAt(this._getComponent() as unknown as HTMLElement);
	}
	...
	async _attachPopupListeners() {
		...
		if (!this.attachedAfterOpened) {
			let suggestionPopover = await this._getSuggestionPopover();
