...
const getPlugins = ({ transpile }) => {
	...
	plugins.push(url({
		...
		publicPath: DEPLOY_PUBLIC_PATH + "/assets/js/ui5-webcomponents/",
	}));
...
const getES6Config = () => {
	return [{
		input: "../../main/bundle.esm.js",
		output: {
			dir: "assets/js/ui5-webcomponents",
			format: "esm",
			sourcemap: true
		},
		moduleContext: (id) => {
...
const getDerivedColorsConfig = (theme) => {
	return {
		input: `../../main/src/themes/${theme}/derived-colors.js`,
		output: {
			dir: `assets/js/ui5-webcomponents/${theme}/`,
			format: "esm",
			sourcemap: true
		},
		plugins: [commonjs()],
