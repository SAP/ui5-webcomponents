...
const getThemeStylesString = (styles: ComponentStylesData) => {
	if (Array.isArray(styles)) {
		return (styles.filter(style => !!style).flat(MAX_DEPTH_INHERITED_CLASSES) as Array<StyleData>).map((style: StyleData) => {
			...
			return typeof currentPackageStyles === "string" ? currentPackageStyles : currentPackageStyles!.content;
		}).join(" ");
	}
...
export default getThemeStylesString;
