...
const createStyle = (data: StyleData, name: string, value = "") => {
	...
	if (shouldUseLinks()) {
		const attributes = {} as ObjectWithDynamicKeys;
		...
		(stylesheet as ObjectWithDynamicKeys)._ui5StyleId = getStyleId(name, value); // set an id so that we can find the style later
...
const updateStyle = (data: StyleData, name: string, value = "") => {
	...
	if (shouldUseLinks()) {
		...
		const stylesheet = document.adoptedStyleSheets.find(sh => (sh as ObjectWithDynamicKeys)._ui5StyleId === getStyleId(name, value));
		if (stylesheet) {
			stylesheet.replaceSync(content || "");
		}
	} else {
...
const hasStyle = (name: string, value = "") => {
	...
	if (document.adoptedStyleSheets) {
		return !!document.adoptedStyleSheets.find(sh => (sh as ObjectWithDynamicKeys)._ui5StyleId === getStyleId(name, value));
	}
...
const removeStyle = (name: string, value = "") => {
	if (shouldUseLinks()) {
		...
		document.adoptedStyleSheets = document.adoptedStyleSheets.filter(sh => (sh as ObjectWithDynamicKeys)._ui5StyleId !== getStyleId(name, value));
	} else {
