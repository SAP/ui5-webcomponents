...
### Wrapping Behavior
The standard list item `<ui5-li>` support text wrapping through the <b>wrapping</b> property. When enabled, long text content (title and description) will wrap to multiple lines instead of truncating with an ellipsis. For very long content, the text is displayed with a "Show More/Show Less" mechanism.
