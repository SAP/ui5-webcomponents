...
- Removed APIs without alternatives: 
  ...
  - Notification List Group (ui5-li-notification-group): The properties `showClose`, `showCounter`, `priority`, the event `close"` and the slot `actions` are removed due to new better notification list design concept.
