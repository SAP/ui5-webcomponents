---
title: The Table Web Component is Now Stable - Here's What's New
tags: [table, experimental]
slug: /stable-table-release
...
pain point being accessibility. Comparing the 1.x Table component with its equivalent UI5 Tables, accessibility features were not as sophisticated
...
3. Improve existing APIs and incorporate new ideas
...
Giving us time to manifest the final API, but most importantly providing us the chance to listen to your feedback and improve upon it.

Finally, thanks to your feedback, we can lift the `ui5-table` out of its experimental state.
...
We've additionally decided to apply ARIA roles such as "row" or "gridcell" directly on the Web Components instead of hiding them in the Shadow DOM, applying
the semantics directly to the Web Component. 
This change in combination with the new structure enables a better Accessibility and Screen Reader Support, as the new Web Component Table now structurally
and semantically resembles the HTML table, ensuring assitive technologies easier processing.
...
is greatly improved in other areas as well.
...
newly implemented keyboard specification allows you to navigate row by row **and** from cell to cell. This update aligns the Web Component Table with its counterparts
in UI5 in terms of keyboard handling.

![Keyboard Handling](./images/keyboard_handling.gif)

Check the [Keyboard Handling Specification](https://help.sap.com/docs/SAPUI5/bc5a64aac808463baa95b4230f221716/10c137d404874350b886cc988aa63c1c.html?locale=en-US) for more information
on the handling.

### Feature Concept

If you use the new `ui5-table` for the first time, you might notice that "core features" of the old Table implementation might be missing, such as growing capabilities or
selection functionality.

This discrepancy is intended, as we wanted to provide a light-weight component for users, that might not need such features.
...
plug-and-play option for more complex features of the Table. This should allow you to keep the Table as light-weight as possible, ensuring only necessary
code/functionality is being used.

The `ui5-table` has a dedicated `features` slot to enable certain features, which are dedicated Web Components. We are currently offering the following features:
...
them to keep the API minimal, clean, and better suited to their respective use cases. This approach also helps manage complexity, as each selection mode has distinct API requirements
and behaviors.
...
the table, and the selection feature itself maintains a `selected` property containing the keys of selected rows. To enable this, you need to provide a `row-key` for your rows uniquely
identifying them.
...
This feature will ensure, that only the rows visible in the current viewport are rendered and updated during scrolling. To achieve this, the `TableVirtualizer` offers a
...
The old Table component used by default "popin mode", a responsive display option optimal for small screen sizes. Defining a `minWidth` for the column, a minimum table width,
allows the column to move into the "popin area", once the table is smaller than the specified `minWidth`.
...
The `ui5-table` newly introduces row actions. The `ui5-table-row` has a dedicated `actions` slot, which allows you to add custom row action to your rows.

There are two row action options:
1. Use [`TableRowActionNavigation`](https://sap.github.io/ui5-webcomponents/components/TableRowActionNavigation/) to define a navigation action.
2. Use [`TableRowAction`](https://sap.github.io/ui5-webcomponents/components/TableRowAction/) to define a custom navigation. Provide a text and icon for your custom action.
...
The Table component is now drag and drop enabled! By utilizing the Table's `move-over` and `move` event, you are able to move table rows.

This allows you to implement features such as row reordering, exchanging data between tables, and even more.
...
- Selection Improvements such as Clear All and better Select All
- Separators
- Row Span
