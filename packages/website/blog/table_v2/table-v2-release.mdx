...
Alongside revamping the selection behavior, we've decided to improve and adapt the growing behavior as well. `TableGrowing` enables growing capabilities and provides to different
growing modes: `Button`, which will render a button that needs to be clicked to load more data, and `Scroll`, which loads more data upon scrolling to the end of the table.

A slight improvement has been made to the `Scroll` behavior: If the table is not scrollable yet, the growing button will be rendered until the table is scrollable. This will always ensure,
