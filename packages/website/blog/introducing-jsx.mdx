...
Until now, Handlebars was the only templating language that could be used. The main goal of the project since the beginning was to have declarative renderers, and Handlebars served this purposse well. There were some drawbacks however.

## Own HBS compiler
Since handlebars originally works with strings only and cannot update the DOM effectively, we had to build our own HBS -> lit-html compiler. This added some technical debt, as not all features from handlerbars were implemented. It also added maintenance effort for a tool we had to update and evolve, when switching to TypeScript for example, or when encountering edge cases not previously considered.

## Development experience
All templating languages are by default not understood by IDEs out of the box.

Code completion is the beggest gap we were seeing - there was simply no way to tell the IDE what to do without writing a plugin, which would be further development effort.
...
Code navigation was also missing - you could not click on a property in a HBS template to navigate to the property definition in the compoment itself.

## JSX

JSX is an embeddable HTML-like syntax that gets transformed to JavaScript. It is written inside JavaScript and transformed to JavaScript, which means the TypeScript compiler can do all of the things it does out of the box - not only typechecking, but also provide code completion suggestions and do the code transformation. IDEs also have TypeScript integration, so this means nothing additional is required.
...
For more implementation defails, you can check the (template docs)[https://sap.github.io/ui5-webcomponents/docs/development/templates/] and the (PR)[https://github.com/SAP/ui5-webcomponents/pull/10046]